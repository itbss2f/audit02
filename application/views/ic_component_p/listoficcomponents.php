...
<div class="breadcrumb pull-right">
    <li>Home</li>
    <?php if($canADD):?>
    <li class="active"><a></a>IC Component Principle</li>
    <?php endif;?>
</div>
    ...
    <div class="col-md-12">
        ...
        <div class="panel panel-inverse">
            <div class="panel-heading">
                ...
                <h4 class="panel-title" style="margin-top: 5px;">IC Component Principle - <div class="btn btn-success btn-xs">Total of <?php echo number_format($total['total_iccomponents'], 0, "", ","); ?> entries</div></h4>
            </div>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($data as $row) : ?>
                                    <tr>
                                        ...
                                        <?php endif;?>
                                        <?php if($canDELETE): ?>
                                        <a href="<?php echo site_url('ic_component_p/removedata').'/'.$row['id']?>" type="submit" title="delete" class="btn btn-xs btn-icon btn-circle btn-danger remove"><i class="fa fa-times"></i></a>
                                        <?php endif;?>
                                        </td>
                                    </tr>
                                    <?php $no += 1; ?>
...
<div id="modal_edit" title="Edit"></div>
<div id="modal_add" title="Add"></div>
...
$(function() {
    ...
    $('.edit').click(function() {
        var $id = $(this).attr('id');
        $.ajax({
          url: "<?php echo site_url('ic_component_p/editdata') ?>",
          type: "post",
          data: {id: $id},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_edit").html($response['editdata']).dialog('open');
              }
           });
        });
        return false;
...
$(function() {
    ...
    $('.add').click(function() {
        $.ajax({
          url: "<?php echo site_url('ic_component_p/newdata') ?>",
          type: "post",
          data: {},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_add").html($response['newdata']).dialog('open');
              }
           });
        });
        return false;
