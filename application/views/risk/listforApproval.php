<div class="breadcrumb pull-right">
    ...
    <li class="active">Risk for Approval</li>
</div>
<!-- begin row -->
    ...
    <div class="col-md-12">
        <div class="panel panel-inverse">
            <div class="panel-heading">
                ...
                <h4 class="panel-title" style="margin-top: 5px;">Risk for Approval - <div class="btn btn-success btn-xs">Total of <?php echo number_format($totalforApprove_risk['totalforApprove_risk'], 0, "", ","); ?> entries</div></h4>
            </div>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($data as $row) : ?>
                                    <tr>
                                        ...
                                        <?php if($canEDIT):?>
                                        <a id="<?php echo $row['id'] ?>" title="edit" class="btn btn-xs btn-icon btn-circle btn-primary r_edit"><i class="fa fa-pencil"></i></a>
                                        <?php endif;?>
                                        <?php if($canDISAPPROVED):?>
                                        <a href="<?php echo site_url('risk/disapproved').'/'.$row['id']?>" title="disapproved" class="btn btn-xs btn-icon btn-circle btn-danger r_disapproved"><i class="fa fa-thumbs-down"></i></a>
                                        <?php endif;?>
                                        <?php if($canAPPROVED):?>
                                        <a id="<?php echo $row['id'] ?>" title="approved" class="btn btn-xs btn-icon btn-circle btn-warning r_approved"><i class="fa fa-thumbs-up"></i></a></td>
                                        <?php endif;?>
                                    </tr>
                                    <?php $no += 1; ?>
                                 <?php endforeach; ?>
...
<div id="modal_viewrisk" title="For Approval"></div>
<div id="modal_editRiskfa" title="Edit Risk for Approval"></div>
...
$(function() {
    ...
    $('.r_edit').click(function() {
        var $id = $(this).attr('id');
        $.ajax({
          url: "<?php echo site_url('risk/editdatafa') ?>",
          type: "post",
          data: {id: $id},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_editRiskfa").html($response['editdatafa']).dialog('open');
              }
           });
        });
        return false;
...
$(function() {
    ...
    $('.r_approved').click(function() {
        var $id = $(this).attr('id');
        $.ajax({
          url: "<?php echo site_url('risk/viewrisk') ?>",
          type: "post",
          data: {id: $id},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_viewrisk").html($response['viewrisk']).dialog('open');
              }
           });
        });
        return false;
