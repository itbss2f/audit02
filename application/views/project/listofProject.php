    ...
    <div class="col-md-12">
        ...
        <div class="panel panel-inverse">
            <div class="panel-heading">
                ...
                <h4 class="panel-title" style="margin-top: 5px;">Project Records - <div class="btn btn-success btn-xs">Total of <?php echo number_format($total['total_projects'], 0, "", ","); ?> entries</div></h4>
            </div>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($data as $row) : ?>
                                    <tr>
                                        ...
                                        <?php endif;?>
                                        <?php if($canDELETE):?>
                                        <a href="<?php echo site_url('project/removedata').'/'.$row['id']?>" type="submit" title="delete" class="btn btn-xs btn-icon btn-circle btn-danger remove"><i class="fa fa-times"></i></a>
                                        <?php endif;?>
                                        <?#php if($canUPLOAD):?>
                                        <a id="<?php echo $row['id']?>" title="upload" class="btn btn-xs btn-icon btn-circle btn-primary upload"><i class="fa fa-upload"></i></a>
                                        <?#php endif;?>
                                        </td>
                                    </tr>
                                    <?php $no += 1; ?>
...
<div id="modal_editProject" title="Edit Project"></div>
<div id="modal_add_Project" title="Add Project"></div>
...
$(".upload").click(function () {
    ...
    if (ans)
    {
    window.location = "<?php echo site_url('project/uploadprojectdata') ?>/"+$id; 
    return true;
    }
    else
    {
...
$(function() {
    ...
    $('.edit').click(function() {
        var $id = $(this).attr('id');
        $.ajax({
          url: "<?php echo site_url('project/editdata') ?>",
          type: "post",
          data: {id: $id},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_editProject").html($response['editdata']).dialog('open');
              }
           });
        });
        return false;
...
$(function() {
    ...
    $('.add').click(function() {
        $.ajax({
          url: "<?php echo site_url('project/newdata') ?>",
          type: "post",
          data: {},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_add_Project").html($response['newdata']).dialog('open');
              }
           });
        });
        return false;
