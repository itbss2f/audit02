...
<div class="breadcrumb pull-right">
    <li>Home</li>
    <li class="active">Department Records</li>
    <li class="active"><a>Department Records</a></li>
</div>
<!-- begin row -->
<div class="row">
    <!-- begin col-12 -->
    <div class="col-md-12">
        ...
        <div class="panel panel-inverse">
            <div class="panel-heading">
                ...
                <h4 class="panel-title" style="margin-top: 5px;">Departments Records - <div class="btn btn-success btn-xs">Total of <?php echo number_format($total['total_depart'], 0, "", ","); ?> entries</div></h4>
            </div>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($data as $row) : ?>
                                    <tr>
                                        ...
                                        <td style="text-align: left;" ><?php echo $row['depart_code'] ?>     </td>
                                        <td style="text-align: left;" ><?php echo $row['name'] ?></td>
                                        <td style="text-align: left;" ><?php echo $row['dept_user_n'] ?></td>
                                        <td style="text-align: left;" ><?php echo $row['dept_user_d']?></td>
                                        <td colspan="2" style="text-align: right;font-size: 15px;">
                                        <?php if ($canEDIT):?>
                                        <a id="<?php echo $row['id']?>" title="edit" class="btn btn-xs btn-icon btn-circle btn-primary edit_depart"><i class=" fa fa-pencil"></i></a>
                                        <?php endif;?>
                                        <?php if($canDELETE):?>
                                        <a href="<?php echo site_url('department/removedata').'/'.$row['id']?>" type="submit" title="remove" class="btn btn-xs btn-icon btn-circle btn-danger remove"><i class="fa fa-times"></i></a>
                                        <?php endif;?>
                                        </td>
                                    </tr>
                                    <?php $no += 1; ?>
...
<div id="modal_add_depart" title="Add Department"></div>
<div id="modal_edit_depart" title="Edit Department"></div>
...
$(function() {
    ...
    $('.add').click(function() {
        $.ajax({
          url: "<?php echo site_url('department/newdata') ?>",
          type: "post",
          data: {},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_add_depart").html($response['newdata']).dialog('open');
              }
           });
        });
        return false;
...
$(function() {
    ...
    $('.edit_depart').click(function() {
        var $id = $(this).attr('id');
        $.ajax({
          url: "<?php echo site_url('department/editdata') ?>",
          type: "post",
          data: {id: $id},
          success:function(response) {
             $response = $.parseJSON(response);
              $("#modal_edit_depart").html($response['editdata']).dialog('open');
              }
           });
        });
        return false;
